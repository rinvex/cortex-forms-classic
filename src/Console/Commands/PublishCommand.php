...
class PublishCommand extends BasePublishCommand
{
     ...
     * @var string
     */
    protected $signature = 'cortex:publish:forms {--f|force : Overwrite any existing files.} {--r|resource=* : Specify which resources to publish.}';
    ...
    public function handle(): void
    {
        ...
        collect($this->option('resource') ?: ['config', 'lang', 'views', 'migrations'])->each(function ($resource) {
            $this->call('vendor:publish', ['--tag' => "cortex/forms::{$resource}", '--force' => $this->option('force')]);
        });
