...
Menu::register('adminarea.sidebar', function (MenuGenerator $menu, Form $form) {
    $menu->findByTitleOrAdd(trans('cortex/foundation::common.cms'), 40, 'fa fa-file-text-o', 'header', [], function (MenuItem $dropdown) use ($form) {
        $dropdown->route(['adminarea.cortex.forms.forms.index'], trans('cortex/forms::common.forms'), 20, 'fa fa-wpforms')->ifCan('list', $form)->activateOnRoute('adminarea.cortex.forms.forms');
    });
});

Menu::register('adminarea.cortex.forms.forms.tabs', function (MenuGenerator $menu, Form $form, FormResponse $formResponse) {
    ...
    $menu->route(['adminarea.cortex.forms.forms.responses', ['form' => $form]], trans('cortex/forms::common.responses'))->ifCan('list', $formResponse)->if($form->exists && array_key_exists('database', $form->actions ?? []));
